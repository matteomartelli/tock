    ...
    pub fn _get_sys_clock_frequency_mhz_no_cache(&self) -> usize {
        match self.get_sys_clock_source() {
            ...
            SysClockSource::PLL => {
                ...
                self.pll.get_frequency_mhz_no_cache(pll_source_frequency).unwrap()
            }
        }
    }
...
pub trait ClocksCtrl {
    ...
    // Extend this to expose additional clocks resources
}

impl<'a, ChipSpecs: ChipSpecsTrait> ClocksCtrl for Clocks<'a, ChipSpecs> {
...
pub mod tests {
    ...
    pub fn test_clocks_struct<ChipSpecs: ChipSpecsTrait>(clocks: &Clocks<ChipSpecs>) {
        ...
        check_and_panic!(HSI_FREQUENCY_MHZ, clocks.get_sys_clock_frequency_mhz(), clocks);
